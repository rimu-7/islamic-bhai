  ...
  return (
    <div>
      ...
      <div className="container mx-auto py-6 space-y-8">
        ...
        {zikrData.sections.map((section) => (
          <div key={section.id} id={section.id} className="space-y-6 scroll-mt-40">
            <h2 className="text-2xl font-semibold text-gray-800 border-b-2 border-gray-200 pb-2">
              {section.title}
            </h2>
            {section.items.map((item) => (
              <Card key={item.id} id={item.id} className="p-4 transition-shadow duration-300 scroll-mt-40">
                <CardContent className="space-y-4">
                  <h3 className="text-lg font-bold text-green-400">
                    {item.name} <span className="text-red-600 text-2xl">{item.reps}</span>
                  </h3>
                  ...
                  <p className="text-gray-700 text-lg italic">{item.translation}</p>
                  {item.note && (
                    <p className="text-lg text-gray-500 bg-gray-100 p-2 rounded-md">📌 {item.note}</p>
                  )}
                </CardContent>
              </Card>
            ))}
            ...
            {(section.info1 || section.info2 || section.info3) && (
              <div className="flex flex-col gap-3">
                {section.info1 && <p className="text-sm text-gray-500 bg-gray-100 p-2 rounded-md">📌 {section.info1}</p>}
                {section.info2 && <p className="text-sm text-gray-500 bg-gray-100 p-2 rounded-md">📌 {section.info2}</p>}
                {section.info3 && <p className="text-sm text-gray-500 bg-gray-100 p-2 rounded-md">📌 {section.info3}</p>}
              </div>
            )}
          </div>
        ))}
